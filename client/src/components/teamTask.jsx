...
function TeamTasklist(){
    const {team_task,users} = useUsercontext()
                ...
                team_task.map((task,index1)=>{
                    return(
                            <tr className="hover:bg-gray-50 transition-colors" key = {index1}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{task.main_name}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{task.team_name}</td>
                                ...
                                { (task.assigned_to==null) && (<form onSubmit={(e)=>{
                                        e.preventDefault();
                                        ...
                                        <select className="text-black bg-white rounded-md" required onChange={(e)=>{
                                            ...
                                            <option value = ""></option>
                                            {
                                                users.map((user,index)=>{
                                                    if(user.team_id === task.assigned_team){
